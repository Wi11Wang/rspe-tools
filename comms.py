...
class Visualiser:
    ...
    def plot_mask(self, mask, alpha=1, show_ticks=False, ax=None):
        """
        Plot a mask image.
    ...
    def view_tomo_stack(self, tomo_stack, figsize=(10, 10), downscale=False):
        ...
        if downscale:
            slice0 = cv2.resize(np.array(tomo_stack[0]), (300, 300), interpolation=cv2.INTER_AREA)
        else:
        ...
        def update(val):
            idx = int(slider.val)
            if downscale:
                slice_img = cv2.resize(np.array(tomo_stack[idx]), (300, 300), interpolation=cv2.INTER_AREA)
            else:
    ...
    def view_mask_stack(self, mask_stack, figsize=(10, 10), downscale=False):
        ...
        if downscale:
            ...
            slice0 = cv2.resize(slice0, (300, 300), interpolation=cv2.INTER_NEAREST)
        else:
            slice0 = mask_stack[0]
        ...
        def update(val):
            ...
            if downscale:
                ...
                slice_img = cv2.resize(slice_img, (300, 300), interpolation=cv2.INTER_NEAREST)
            else:
